...
export function IntroPaint() {
  ...
  // Cierre automático + saltar con interacción.
  useEffect(() => {
    if (!show) return
    const dismiss = () => setLeaving(true)
    const t = window.setTimeout(dismiss, reduce ? 750 : 1850)
    ...
    <AnimatePresence onExitComplete={() => setShow(false)}>
      {!leaving && (
        ...
        >
          ...
          {/* wordmark con revelado tipo "pintado" (clip wipe) */}
          <motion.h1
          ...
          {/* pincelada roja que se pinta debajo */}
          <svg
            viewBox="0 0 600 90"
            className="mt-3 h-auto w-[70vw] max-w-[600px]"
            fill="none"
            aria-hidden
          >
            <defs>
              <filter id="mb-brush" x="-15%" y="-60%" width="130%" height="220%">
                <feTurbulence type="fractalNoise" baseFrequency="0.018 0.12" numOctaves="2" seed="9" result="noise" />
